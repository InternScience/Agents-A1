...
def download_nltk_resources():
    """Download required NLTK resources with visible progress."""
    resources = [
    ...
    print("[NLTK] Checking required resources...", flush=True)
    for resource_name, resource_path in resources:
        try:
            nltk.data.find(resource_path)
            print(f"[NLTK] {resource_name} already available", flush=True)
        except LookupError:
            print(f"[NLTK] Downloading {resource_name}...", flush=True)
            download_ok = nltk.download(resource_name, quiet=False)
            if not download_ok:
                raise RuntimeError(f"Failed to download NLTK resource: {resource_name}")
            print(f"[NLTK] Downloaded {resource_name}", flush=True)
    print("[NLTK] Resource check complete.", flush=True)
